<div class="row">
	<div class="col-md-12">
		<div class="total-roll">
			...
			<a class="btn btn-default link-to-unsave" data-to="<?= base_url("admin/productions/cpp/tab/$tabName/$year/$month/$day/false") ?>" data-target="<?= ".cpp-body" ?>"><i class="fa fa-refresh"></i></a>
		</div>
...
<script>
	...
	const CHANGE_DESC_URL = `${BASE_URL}admin/productions/cpp/desc`;
	const CHANGE_CORONA_URL = `${BASE_URL}admin/productions/cpp/corona`;
	const CHANGE_HAZE_URL = `${BASE_URL}admin/productions/cpp/haze`;
	let tabName = '<?= $tabName ?>';
	...
	function loadData(status) {
		...
		let limit = $("#limit").val();
		let page = 1;
		const url = encodeURI(`${BASE_URL}admin/productions/cpp/cpp-table?year=${dt[2]}&month=${dt[1]}&limit=${limit}&page=${page}&kode_mesin=${machineName}&where_in:status=${status}`);
		setContentLoader(".cpp-table");
		loadContent(url, ".cpp-table");
	}
	...
	function loadDataDay(status) {
		...
		let limit = $("#limit").val();
		let page = 1;
		const url = encodeURI(`${BASE_URL}admin/productions/cpp/cpp-table?tgl_input=${tgl}&limit=${limit}&page=${page}&kode_mesin=${machineName}&where_in:status=${status}`);
		setContentLoader(".cpp-table");
		loadContent(url, ".cpp-table");
	}
	...
	function setHaze(element, value, id) {
		if (!isActive) {
			$(".haze-editor").removeClass();
			$(`#${element}`).replaceWith(
				`<input autofocus data-elid="${element}" data-id="${id}" class="haze-editor" type="number" value="${value}" />`
			);
		}
	}

	$(document.body).on("change", ".haze-editor", function() {
		const element = $(this);
		let value = element.val();
		let id = element.data("id");
		let elementId = element.data("elid");
		let url = CHANGE_HAZE_URL;
		let data = {
			id: id,
			value: value
		}

		$(".haze-editor").replaceWith(
			`<a id="${elementId}" onclick="setHaze('${elementId}', '${value}', '${id}')">${value}</a>`);
